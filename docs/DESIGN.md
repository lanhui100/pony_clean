...
## ADR-014: 空间分析扫描准确性 —— 关闭 skip_hidden + Top-K 有界收集 + 单次全量降序事件

**状态**: 已采纳并实现（TASK-033，2026-08-24）

**上下文**: 清理 Tab「空间分析」显示的文件数目与真实磁盘不符。真机交叉验证
（jwalk 扫描 vs 独立 std::fs 地面真值，66 万+ 文件）定位出五个根因：
1. jwalk 默认 `skip_hidden=true` 按「名字以 . 开头」跳过条目（跨平台名字判定而非 Windows
   隐藏属性），`.rustup`/`.cargo`/`.bun` 等开发缓存目录被整棵静默排除（≥100MB 大文件 78 vs 真值 97）；
2. 每 50 个大文件 `split_off` 分批推送后，`max_files` 截断判断作用在余量上——上限 ≥50 永不触发，
   <50 时保留的是先遍历项而非最大项；
3. 结束时只对尾批排序，全局顺序为并行遍历遭遇序，「仅展示最大的 20 个」失真；
4. 进度每 200 个节流且终值不补发，前端「已扫描 N 个文件」欠账最多 199；
5. 函数返回值只剩分批尾批，与事件流数据不一致。

**决策**:
1. disk 模块三处 WalkDir 显式 `.skip_hidden(false)`；SKIP_DIRS 名单剪枝与系统 hive 名单照常生效，
   `.git` 内大文件仍被排除（回归测试锁定）。
2. 新增 `TopLargest` 最小堆收集器（容量 = max_files，(size, path) 全序保证截断集合确定性）：
   遍历全程不中断（保进度计数与 DirUsage 完整），内存上界钉死在 O(cap) 而非全树驻留。
3. `LargeFiles` 事件改为结束时一次性全量发送（降序、≤ max_files 条、与返回值一致）；放弃扫描中
   分批流式——前端大文件区块本就 done 态才渲染，流式批次从未产生 UI 价值，反成缺陷来源。
4. Done 前强制补发精确 Progress 终值（带去重守卫），前端计数不再欠账。

**理由**: 展示准确性是清理工具的信任基线；cap 语义修正后「Top-1000 最大项」承诺可验证；
单事件消除整类顺序/重复缺陷面。备选的 Vec 全量收集 + truncate 方案更简单但失去 pub API 下的
内存上界保证，不采纳。

**影响**:
- `crates/pony_core/src/disk.rs`（skip_hidden / TopLargest / 事件契约 / 文档）
- 新增 `crates/pony_core/tests/integration_disk.rs`（夹具地面真值 + #[ignore] 真机冒烟）
- 前端零改动（useDisk concat 对单事件恒等）
- 已知遗留：cleaner.rs 两处 WalkDir 同受默认 skip_hidden 影响 → TASK-034 跟踪（涉删除候选扩大，
  需独立评估）；cleaner `Done.total_items` 触顶多计 1（UI 不展示）→ TASK-035 跟踪。

**留待手动 QA**: 应用内实际扫描一次，核对大文件摘要与目录占用 Top 榜。
