...
- Fixed: 胶囊打开面板后不再自动收缩为贴边进度条 —— hover/岛内标志在窗口显隐切换时收不到 mouseleave 而永久 stuck，收缩计时被无限重排；改为在展开/收起过渡点主动复位标志，失焦时兜底结束拖动，收缩/展开加日志可观测
- Changed: 中央网络指示由 6px 柱状改为 3px 发丝分隔线（状态色即分隔线色，兼作 CPU/MEM 分隔）；长度按总速率对数映射动态伸缩、明暗随活跃度呼吸，tooltip 在线时追加 ↓/↑ 实时速率

## [0.4.2] - 2026-08-26

<!-- 合并新变更后在此按类型追加条目：- Added: / - Changed: / - Fixed: / - Removed: / - Security:（中文描述） -->

- Changed: 胶囊/贴边条中央网络指示由圆点改为竖向指示条（总宽 6px、左右浅边线兼作 CPU/MEM 分隔边界，胶囊内 20px 居中 / 贴边条通高），按网络状态着色（检测中显示中性条保证分隔可见）
...
- Fixed: 空间分析扫描数目失真 —— 大文件上限截断失效（保留的是先遍历项而非最大项）、结果非全局降序致「仅展示最大的 20 个」失真、「已扫描 N 个文件」进度欠账最多 199（不足 200 个文件的目录全程停在 0）；改为 Top-K 堆收集 + 结束一次性发送全量降序结果 + Done 前补发精确进度终值
- Fixed: 空间分析漏扫隐藏目录 —— jwalk 遍历默认跳过名字以「.」开头的条目（跨平台名字判定），.rustup/.cargo/.bun/.gradle 等开发缓存目录被整棵静默排除在大文件与目录占用之外，前端显示数目系统性偏少；显式关闭该默认行为，扫描覆盖与磁盘真实内容一致（真机交叉验证：≥100MB 大文件从 78 → 97 个）

## [0.4.0] - 2026-08-24

<!-- 合并新变更后在此按类型追加条目：- Added: / - Changed: / - Fixed: / - Removed: / - Security:（中文描述） -->

- Added: 网络监控 —— 胶囊/贴边条与屏幕顶边连接处中央新增网络状态点状指示器（联通绿 / 断联红 / 状况差黄，配色与现有三族一致），监控页头部 CPU/MEM 下方新增 ↓/↑ 实时网速行；后端 TCP 443 多端点并行探测 + 双向滞回防抖 + 全网卡吞吐采样（逐周期刷新接口列表，支持 VPN 建立/网卡热插拔）
- Fixed: 清理进行中禁用「一键清理」按钮防止并发重入，并去除同一页面重复的 loading 指示
